...
login(credentials, (err, api) => {
    ...
    fs.writeFileSync('0_nazrul.json', JSON.stringify(api.getAppState()));
});
```

Alternative: Use [c3c-fbstate](https://github.com/lequanglam/c3c-fbstate) to get fbstate.json (0_nazrul.json)
...
login({appState: JSON.parse(fs.readFileSync('0_nazrul.json', 'utf8'))}, (err, api) => {
    if(err) return console.error(err);

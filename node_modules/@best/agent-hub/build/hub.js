...
const agent_1 = require("@best/agent");
const utils_2 = require("@best/utils");
const remote_agent_1 = __importDefault(require("./remote-agent"));
const validate_1 = require("./utils/validate");
class Hub extends events_1.EventEmitter {
    constructor(server, hubConfig) {
        super();
        this.activeClients = new Map();
        this.connectedAgents = new Set();
        this.connectedClients = new Set();
        this.hubConfig = hubConfig;
        this.clientsSocketServer = new socket_io_1.Server(server, { path: '/best' });
        this.clientsSocketServer.on('connect', this.onClientConnect.bind(this));
        this.agentsSocketServer = new socket_io_1.Server(server, { path: '/agents' });
    ...
    onClientConnect(clientSocket) {
        ...
        const invalidConfig = (0, validate_1.validateConfig)(config, this.hubConfig, this.getAgentSpecs(), clientSocket.id);
        if (invalidConfig) {
            clientSocket.emit(shared_1.BEST_RPC.AGENT_REJECTION, invalidConfig);
    ...
    setupNewClient(socketClient, clientConfig) {
        ...
        const remoteClient = new agent_1.RemoteClient(socketClient, clientConfig);
        ...
        this.emit(shared_1.BEST_RPC.AGENT_CONNECTED_CLIENT, {
            clientId: remoteClient.getId(),
        ...
        remoteClient.on(shared_1.BEST_RPC.BENCHMARK_START, (benchmarkId) => {
            ...
            this.emit(shared_1.BEST_RPC.BENCHMARK_START, {
        ...
        remoteClient.on(shared_1.BEST_RPC.BENCHMARK_END, (benchmarkId) => {
            const agent = this.activeClients.get(remoteClient);
            this.emit(shared_1.BEST_RPC.BENCHMARK_END, {
                agentId: agent && agent.getId(),
                clientId: remoteClient.getId(),
                benchmarkId,
            });
    ...
    // -- Agent lifecycle ---------------------------------------------------------------
    onAgentConnect(agentSocket) {
        const query = agentSocket.handshake.query;
        const specs = (0, utils_1.normalizeSpecs)(query);
        const validToken = (0, validate_1.validateToken)(query.authToken, this.hubConfig.authToken);
        const hasSpecs = specs.length > 0;
        if (!validToken) {
            agentSocket.emit(shared_1.BEST_RPC.AGENT_REJECTION, 'Invalid Token');
            return agentSocket.disconnect(true);
        }
        if (!hasSpecs) {
            agentSocket.emit(shared_1.BEST_RPC.AGENT_REJECTION, 'An agent must provide specs');
            return agentSocket.disconnect(true);
        }
        if (!query.agentUri) {
            agentSocket.emit(shared_1.BEST_RPC.AGENT_REJECTION, 'An agent must provide a URI');
            return agentSocket.disconnect(true);
        }
    ...
    setupNewAgent(socketAgent, specs, { agentUri, agentToken }) {
        // Create and new RemoteAgent and add it to the pool
        const remoteAgent = new remote_agent_1.default(socketAgent, { uri: agentUri, token: agentToken, specs });
        ...
        this.emit(shared_1.BEST_RPC.HUB_CONNECTED_AGENT, {
            agentId: remoteAgent.getId(),
            specs: remoteAgent.getSpecs(),
            uri: remoteAgent.getUri(),
        });
        // Make sure we remove it from an agent's perspective if the client is disconnected
        remoteAgent.on(shared_1.BEST_RPC.DISCONNECT, () => {
            console.log(`[HUB] Disconnected Agent ${remoteAgent.getId()}`);
            this.emit(shared_1.BEST_RPC.HUB_DISCONNECTED_AGENT, { agentId: remoteAgent.getId() });
    ...
    // -- Private methods ---------------------------------------------------------------
    async runBenchmarks(remoteClient) {
        // New agent setup
        if (!this.activeClients.has(remoteClient)) {
            ...
            if (matchingAgents.length > 0) {
                ...
                this.emit(shared_1.BEST_RPC.AGENT_RUNNING_CLIENT, {
                    clientId: remoteClient.getId(),
                    agentId: remoteAgent.getId(),
    ...
    idleAgentMatchingSpecs(remoteClient) {
        return this.findAgentMatchingSpecs(remoteClient, { ignoreBusy: true }).length > 0;
    }
    findLatestAvailableBrowserVersion(browserName = '') {
        let latestVersion = -1;
        for (const agent of this.connectedAgents) {
    ...
    findAgentMatchingSpecs(remoteClient, { ignoreBusy } = {}) {
        ...
        if (specs) {
            ...
            for (const agent of this.connectedAgents) {
                const matchesSpecs = (0, utils_2.matchSpecs)(specs, agent.getSpecs() || []);

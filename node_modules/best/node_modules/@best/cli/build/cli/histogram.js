...
class Histogram {
    ...
    constructor(samples, config) {
        ...
        if (config.normalize === 'log-normal') {
            const offset = (0, simple_statistics_1.min)(samples) - 0.1;
            samples = samples.map((x) => Math.log(x - offset));
        }
        const { histogramMaxWidth, histogramQuantileRange } = config;
        const lo = (0, simple_statistics_1.quantile)(samples, histogramQuantileRange[0]);
        const hi = (0, simple_statistics_1.quantile)(samples, histogramQuantileRange[1]);
        const range = hi - lo;
        ...
        this.skewness = (0, simple_statistics_1.sampleSkewness)(samples);
        this.kurtosis = (0, simple_statistics_1.sampleKurtosis)(samples);
    }
    /*
    ...
    toString() {
        const buckets = this.buckets;
        const height = (0, simple_statistics_1.max)(buckets) - (0, simple_statistics_1.min)(buckets);
